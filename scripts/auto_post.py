...
CATEGORIES = [
    {
        "name": "자동 수익 투자",
        "prompt": "파이어족, ETF 적립식 투자, 배당주 등 초보도 따라할 수 있는 자동 수익 투자법. 실제 종목명/ETF명 포함. 검색 키워드: 배당투자, ETF추천, 적립식투자, 파이어족",
        "img": "money investment finance savings, minimal clean photography",
        "trend_kw": ["ETF", "배당", "투자", "파이어", "적립식", "금리", "주식", "재테크"]
    },
    {
        "name": "초간단 시사",
        "prompt": "국내외 경제·정치·사회 이슈를 일반인이 3분 안에 이해할 수 있게 설명. 최근 뉴스 흐름 반영. 검색 키워드: 경제뉴스, 시사상식, 금리, 환율",
        "img": "newspaper economy news world, modern editorial photography",
        "trend_kw": ["경제", "금리", "환율", "뉴스", "정치", "사회", "시사", "물가"]
    },
    {
        "name": "웰니스 자기관리",
        "prompt": "최신 웰니스 트렌드(수면, 루틴, 번아웃, 식단 등)를 쉽게 설명하고 바로 적용 가능한 실용 가이드. 검색 키워드: 웰니스, 자기관리, 번아웃, 수면루틴",
        "img": "wellness self care calm morning routine, soft natural light",
        "trend_kw": ["wellness", "health", "sleep", "stress", "meditation", "건강", "수면", "번아웃", "웰니스"]
    },
    {
        "name": "2030 핫플 여행",
        "prompt": "20~30대가 좋아하는 국내 핫플레이스, 감성 카페, 여행 코스 소개. SNS에서 뜨는 곳 위주. 검색 키워드: 국내여행, 핫플레이스, 감성카페, 당일치기",
        "img": "aesthetic korea cafe travel trip, instagram style photography",
        "trend_kw": ["여행", "핫플", "카페", "국내여행", "당일치기", "감성", "맛집"]
    },
    {
        "name": "철학 기반 자기계발",
        "prompt": "니체, 공자, 스토아 철학 등 고대~현대 철학/사상을 중학생도 이해하는 수준으로 현대 실생활에 적용하는 자기계발 칼럼. 검색 키워드: 자기계발, 철학, 니체, 스토아철학, 인간관계",
        "img": "philosophy books thinking quiet study, warm moody photography",
    ...
    for url in sources:
        try:
            ...
            if r.status_code == 200:
                ...
                for t in titles[:15]:
                    t = t.strip()
                    if any(kw in t.lower() for kw in [k.lower() for k in cat_kw]):
                        keywords.append(t)
        except Exception:
            pass
    return keywords[:15]
...
def get_access_token():
    ...
    print("=" * 60)
    print("🎉 토큰 발급 성공! GitHub Secret WP_ACCESS_TOKEN 에 저장:")
    print(token)
    print("=" * 60)
    return token

# ── 이미지 HTML 생성 (Pollinations) ───────────────────
...
def generate_post(category, posted_titles, trend_hints):
    ...
    prompt = f"""오늘({today}) '{category['name']}' 카테고리 블로그 글을 써라.
주제 방향: {category['prompt']}

최신 트렌드 참고 (이걸 소재로 활용하거나 영감으로만 써도 됨):
{trend_text}
...
[글쓰기 규칙]
- 블로그 운영자가 직접 쓰는 것처럼 자연스럽게
- 특정 직업·나이·성별 설정 없이 누구나 공감 가능하게
- 짧은 문장과 긴 문장을 섞어 리듬감 있게
- 솔직한 한두 마디 자연스럽게 섞기 (예: "사실 나도 처음엔 헷갈렸다")
- "~입니다" 반복 금지. "~더라", "~거든요" 같은 자연스러운 말투도 섞기
- 뻔한 마무리("결론적으로", "지금 바로") 금지. 여운 있게 끝내기
- 과장·광고 느낌 금지. 솔직하고 담백하게
- 길이: 800~1100자
- HTML 형식: 소제목 <h2>, 문단 <p>, 필요시 <ul><li>
- SEO: 핵심 키워드 3~4회 자연스럽게 포함
...
def main():
    ...
    for cat in CATEGORIES:
        try:
            ...
            posted[cat["name"]] = posted[cat["name"]][-50:]  # 카테고리당 최근 50개 유지
